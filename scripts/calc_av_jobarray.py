...
if len(sys.argv) < 2:
    ...
    clusters_to_process = clusters[x:x+11]
    
    for cluster in clusters_to_process:

        # Initialize gdata as an empty gene expression DataFrame
        gdata = pd.DataFrame(index=cell[cell.cluster == cluster].index, columns=genes.index)
        ...
        for matindex in matrices.index :
            ...
            exp = exp[ exp.index.isin(cell[cell.cluster == cluster].index) ]
        ...
        file = f"{download_base}/metadata/averages/20230830/median/{clustername}.csv"
        ...
        file = f"{download_base}/metadata/averages/20230830/mean/{clustername}.csv"
